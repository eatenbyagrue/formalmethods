...
to compute-change-please
  ...
  if not ( 0 <= expected-change and expected-change <= 1) [
    show "Number between 0 and 1 expected"
    stop
  ]

  show compute-change (100 - int (expected-change * 100))
end

to-report compute-change [remaining-change]
  ;; Finds the least amount of coins that add up to
  ;; @remaining-change

  ;; Create Empty List
  let coins-to-report []

  ;; Types of coins already sorted by $$.
  let coin-types [1 2 5 10 20 50]
  ...
  while [remaining-change > 0] [
    ...
    set remaining-change (remaining-change - largest-coin)

  ]
  report coins-to-report
end
@#$#@#$#@
GRAPHICS-WINDOW
1407
